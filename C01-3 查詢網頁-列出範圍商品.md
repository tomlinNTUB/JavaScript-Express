...
### 測試方式
```
https://自己的app名稱.herokuapp.com/product/range/10/5
```

### 執行結果
![GitHub Logo](/imgs/B01-3.jpg)
...
```
 <web>
   ...
   |      |__ fetchOneProduct.ejs   (前一講)
   |      |__ fetchRangeOfProducts.ejs   (由網頁樣板的index.ejs複製並修改)   
   |
   |__ <routes>
   |      |__ <utility>  
   |      |       |__ asyncDB.js     (自行增加)
   ...
   |      |__ fetchOneProduct.js       (前一講)
   |      |__ fetchRangeOfProducts.js  (自行增加)   
   |
   |__ app.js   (修改)
   |__ package.json    
```
...
var fetchRangeOfProducts = async function(start=0, nums=10){
    ...
    await query('SELECT * FROM product LIMIT $2 OFFSET $1', [start, nums])
        .then((data) => {
            result = data.rows;  
        }, (error) => {
            result = null;
...
//接收GET請求
router.get('/:start/:nums', function(req, res, next) {
    var start = Number(req.params.start);  //開始記錄數, 轉數字
    var nums = Number(req.params.nums);    //取出筆數, 轉數字

    products.fetchRangeOfProducts(start, nums).then(data => {
        if (data.length > 0){
            res.render('fetchRangeOfProducts', {items:data});  //將資料傳給顯示頁面
        }else{
...
var fetchRangeOfProducts = require('./routes/fetchRangeOfProducts');
...
app.use('/product/range', fetchRangeOfProducts);
