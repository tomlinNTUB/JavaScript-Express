...
### 測試方式
```
http://localhost:3000/product/range/10/5
```
...
router.get('/:start/:nums', function(req, res, next) {
    ...
    products.fetchRangeOfProducts(start, nums).then(d => {
        if (d.data.length > 0){
            res.render('fetchRangeOfProducts', {items:d.data});  //將資料傳給顯示頁面
        }else{
